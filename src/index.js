...
class RegtestUtils {
    ...
    async faucet(address, value) {
        const requester = _faucetRequestMaker('faucet', 'address', this.dhttp, this._APIURL, this._APIPASS, this);
    ...
    async faucetComplex(output, value) {
        ...
        const requester = _faucetRequestMaker('faucetScript', 'script', this.dhttp, this._APIURL, this._APIPASS, this);
        const faucet = _faucetMaker(this, requester);
...
function _faucetMaker(self, _requester) {
    return async (address, value) => {
        ...
        while (_unspents.length === 0) {
            ...
            const txId = await _requester(address, value).then(v => v, // Pass success value as is
            async (err) => {
